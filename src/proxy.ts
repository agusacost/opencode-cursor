...
function evictStaleConversations(): void {
 ...
 // Hard cap: evict oldest entries when over the limit
 if (conversationStates.size > MAX_CONVERSATIONS) {
 const sorted = [...conversationStates.entries()].sort(
 ([, a], [, b]) => a.lastAccessMs - b.lastAccessMs,
 );
 const toDelete = sorted.slice(0, conversationStates.size - MAX_CONVERSATIONS);
 for (const [key] of toDelete) conversationStates.delete(key);
 }
}
...
let proxyServer: ReturnType | undefined;
let proxyPort: number | undefined;
let proxyAccessTokenProvider: (() => Promise) | undefined;
...
export async function startProxy(
 ...
 return new Response(
 JSON.stringify({ error: { message: "Request body too large", type: "invalid_request_error" } }),
...
function decodeMcpArgsMap(args: Record): Record {
...
const MAX_THINKING_TAG_LEN = 16; // is 15 chars
...
function handleExecMessage(
 ...
 // Unknown exec type — log only when debug mode is active
 if (process.env.DEBUG_CURSOR_PROXY) console.error(`[proxy] unhandled exec: ${execCase}`);
}
...
function startBridge(
 accessToken: string,
 requestBytes: Uint8Array,
): { bridge: ReturnType; heartbeatTimer: NodeJS.Timeout } {
 const bridge = spawnBridge({
 accessToken,
